...
exports.llmRouter = (0, express_1.Router)();
...
exports.llmRouter.post('/api/chat', async (req, res) => {
    const { messages, config, tools: providedTools } = req.body;
    if (!config?.apiKey) {
    ...
    try {
        ...
        if (tools.length === 0) {
            try {
                tools = await (0, mcp_proxy_1.mcpListTools)();
            }
            catch (err) {
...
// ── Anthropic ──
async function chatAnthropic(messages, tools, config, sendEvent) {
    ...
    let llmMessages = messages.map(m => ({
        role: m.role,
        content: m.content,
    }));
    for (let i = 0; i < MAX_TOOL_LOOPS; i++) {
        ...
        for (const block of response.content) {
            if (block.type === 'text') {
                ...
                try {
                    const result = await (0, mcp_proxy_1.mcpCallTool)(block.name, block.input);
...
// ── OpenAI ──
async function chatOpenAI(messages, tools, config, sendEvent) {
    ...
    for (let i = 0; i < MAX_TOOL_LOOPS; i++) {
        ...
        for (const tc of msg.tool_calls) {
            ...
            try {
                const result = await (0, mcp_proxy_1.mcpCallTool)(tc.function.name, args);
                const resultText = typeof result === 'string' ? result : JSON.stringify(result);
